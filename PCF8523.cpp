...
//    tmp = rtcReadReg(PCF8523_CONTROL_2);
//    tmp|=bit(PCF8523_CONTROL_2_CTAF_BIT)|bit(PCF8523_CONTROL_2_CTAIE_BIT);
//   rtcWriteReg(PCF8523_CONTROL_2,tmp);
//}
...
void PCF8523::rtcStop(void)
{
	...
	tmp |= bit(PCF8523_CONTROL_1_STOP_BIT);	// Freeze the Counter
...
void PCF8523::rtcStart(void)
{
	...
	tmp &= ~bit(PCF8523_CONTROL_1_STOP_BIT);	// Start the Counter
...
void PCF8523::setTimer1(eTIMER_TIMEBASE timebase, uint8_t value)
{
	...
	tmp &= ~bit(PCF8523_CONTROL_2_CTAF_BIT);	// Clear the Timer A flag
	tmp |= bit(PCF8523_CONTROL_2_CTAIE_BIT);	// Enable Timer A interrupt
	...
	tmp |= bit(PCF8523_TMR_CLKOUT_CTRL_TAM_BIT);	// /INT line is pulsed
...
void PCF8523::setTimer2(eTIMER_TIMEBASE timebase,uint8_t value)
{
  ...
  tmp = rtcReadReg(PCF8523_CONTROL_2);
	...
	tmp &= ~bit(PCF8523_CONTROL_2_CTBF_BIT);	// Clear the Timer B flag
	tmp |= bit(PCF8523_CONTROL_2_CTBIE_BIT);	// Enable Timer B interrupt
	...
	tmp |= bit(PCF8523_TMR_CLKOUT_CTRL_TBM_BIT);	// /INT line is pulsed
	tmp |= 0x01;									// Enable	
...
void PCF8523::ackTimer2(void)
{
  ...
  tmp = rtcReadReg(PCF8523_CONTROL_2);
	...
	tmp &= ~bit(PCF8523_CONTROL_2_CTBF_BIT);	// Clear the Timer A flag
...
uint8_t PCF8523::clearRtcInterruptFlags() {
  uint8_t rc2 = rtcReadReg(bit(PCF8523_CONTROL_2) & (bit(PCF8523_CONTROL_2_SF_BIT) | bit(PCF8523_CONTROL_2_AF_BIT)));
...
void PCF8523::rtcCapSelect(eCAP_SEL value)
{
  ...
  {
      // Clear Cap_sel bit
      tmp &= ~bit(PCF8523_CONTROL_1_CAP_SEL_BIT); 
...
void PCF8523::setTwelveTwentyFourHour(e12_24 mode)
{
  ...
  {
      // Clear 12_24 flag
      tmp &= ~bit(PCF8523_CONTROL_1_1224_BIT);  
  }
  else 
  {
      // 12 hour Mode
...
e12_24 PCF8523::getTwelveTwentyFourHour(void)
{
  uint8_t tmp;
  tmp = rtcReadReg(PCF8523_CONTROL_1);
  if(tmp & bit(PCF8523_CONTROL_1_1224_BIT))
  {
     return eTWELVEHOUR;
  }
  else
